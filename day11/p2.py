...
def find_out_monkey_business(input):
    ...
    # this is necessary to allow computation of excessively large numbers
    magic_mod = math.prod([monkey.divisor for monkey in monkeys])
    for _ in range(ROUNDS):
        round(monkeys, magic_mod)
    # return monkeys
    activity_levels = sorted([monkey.activity for monkey in monkeys])
    return activity_levels[-1] * activity_levels[-2]


def round(monkeys, magic_mod):
